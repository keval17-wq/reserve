...
const Sidebar = ({
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div
        className={`hidden md:block fixed top-0 left-0 h-screen z-40 bg-white text-slate-800 shadow-2xl border-r transition-all duration-300 ease-in-out ${isCollapsed ? 'w-20' : 'w-64'
          }`}
      >
...
const SidebarLink: React.FC<SidebarLinkProps> = ({
  ...
  return (
    <Link
      href={href}
      className={`flex items-center mx-3 rounded-lg font-medium group transition-all duration-200 ${active
...
const MobileLink = ({
  ...
  <Link
    href={href}
    className={`block w-full px-4 py-2 rounded-lg text-sm font-medium transition ${active
